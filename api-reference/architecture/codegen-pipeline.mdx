...
<CardGroup cols={3}>
  ...
  <Card title="After: 1 Source of Truth" icon="circle-check" color="#16a34a">
    One `.smithy` model file generates **everything** — Rust server traits,
    TypeScript SDK, and DTO types.
  </Card>
...
<Steps>
  <Step title="Define the contract in Smithy IDL">
    API shapes, operations, and errors are described once in `.smithy` files under `smithy/model/`. The service is `in.aarokya.api#AarokyaService`.
