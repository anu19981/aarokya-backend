...
This is the primary flow for Namma Yatri drivers. After a one-time consent, the system automatically tops up the wallet and purchases insurance daily — no manual intervention needed.
...
sequenceDiagram
    ...
    participant NHCT as NHCT<br/>(Aarokya Fund, Transcorp)

    rect rgba(34, 197, 94, 0.15)
        Note over U,NY: Phase 1 — One-time Consent
        U->>AK: Consent for ₹20/day autopay
        AK->>NY: Link to existing UPI mandate (₹35 ceiling)
        NY-->>AK: Consent registered
    end

    rect rgba(59, 130, 246, 0.15)
        Note over AK,W: Phase 2 — Daily Wallet Top-Up (CRON)
        AK->>NY: Execute autopay ₹20
        NY->>BANK: Debit ₹20 (existing mandate)
    ...
    rect rgba(168, 85, 247, 0.15)
        Note over AK,NHIL: Phase 3 — Auto Insurance Purchase (CRON)
        AK->>AK: Queue for insurance (batch)
        AK-->>U: Policy issuance initiated
        AK->>NHIL: Create policy (auto/manual)
        NHIL-->>AK: Policy Issued
        AK-->>U: 🔔 Notification: Policy Issued
    end

    rect rgba(245, 158, 11, 0.15)
        Note over NY,NHCT: Settlement
        NY->>NHCT: Settle wallet funds
        NHCT->>NHIL: Transfer premium (at intervals)
    end
```
