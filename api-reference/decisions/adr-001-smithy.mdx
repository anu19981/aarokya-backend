...
```mermaid
flowchart TD
    S["📄 Smithy IDL<br/>(one model, one truth)"]

    S -->|"smithy-rs"| R["🦀 Rust server stubs<br/>(aarokya_server_api crate)"]
    S -->|"smithy-typescript"| T["📦 TypeScript client<br/>(@aarokya/api-client)"]
    S -->|"smithy-dto-gen binary"| D["api_models/*.rs + common_enums<br/>+ dto-ts/sdk.ts (shared DTO types)"]

    R --> I["Actix-web handlers<br/>compiler enforces contract"]
    T --> E["Expo app<br/>compiler enforces contract"]
