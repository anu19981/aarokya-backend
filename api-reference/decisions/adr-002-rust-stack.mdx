...
```mermaid
flowchart LR
    A["Diesel<br/>Chosen"] -->|"compile-time<br/>SQL verification"| B["SQL errors caught<br/>before runtime"]
    C["SQLx"] -->|"runtime query<br/>verification"| D["SQL errors at<br/>startup or runtime"]
    E["SeaORM"] -->|"async-native<br/>but less mature"| F["Larger API surface,<br/>younger ecosystem"]
...
- For financial data (PBA account balances, funding orders, mandate executions), this level of safety is worth the synchronous API trade-off
...
│   │   ├── src/
│   │   │   ├── core/     # Business logic (auth, user, account, insurance_policy, consultation, ...)
...
│   ├── common_enums/     # Shared enums (Gender, AccountStatus, MandateStatus, ...)
