...
An account stores a user's external account reference (`provider_account_id`, e.g. a bank account number). All routes are scoped under `/users/{user_id}/accounts`. Accounts are created with `status = pending` by default.
