...
```json Preview Response 200
{
  ...
  "plan_code": "2A",
  "coverage_amount": { "value": 5000.00, "currency": "INR" },
  "duration": { "years": 1 },
  ...
  "premium_amounts": { "daily": 1000, "annual": 360000, "currency": "INR" },
...
```json Create Response 201
{
  ...
  "documents": {
    "health_cards": {},
    "customer_information_sheet": "https://cdn.example.com/assets/customer_information_sheet.pdf"
  }
}
```
</CodeGroup>
...
| `documents.customer_information_sheet` | string? | Static CDN (CloudFront) URL for the Customer Information Sheet. Omitted when not configured. Fetch directly from the CDN — the backend does not proxy it. |

<Note>
  The dynamic document URLs (`health_cards`, `enrollment_form`, `policy_certificate`) appear **only once the policy is downloadable** (status `ISSUED` and subsequent renewal states). Before then, `health_cards` is `{}` and the two URL fields are absent. The `customer_information_sheet` is status-independent. The **pre-purchase** enrollment form preview is a separate endpoint (`preview_enrollment_form`) and is not part of `documents`.
</Note>
