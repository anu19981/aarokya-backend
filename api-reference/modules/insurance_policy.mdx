...
Transitions fall into two independent groups:

- **Human decisions** (admin/provider via `PATCH .../status`) — a guarded state machine; a rejected pair returns `IP_1021`:

  | From | Allowed `to` |
  |---|---|
  | `PENDING` | `APPROVED`, `REJECTED` |
  | `APPROVED` | `ISSUED` (requires `external_policy_id` + `start_date` + `end_date`) |
  | `ISSUED` | `REQUIRES_CUSTOMER_RENEWAL_IN_GRACE`, `REQUIRES_CUSTOMER_RENEWAL`, `REQUIRES_POLICY_REISSUANCE`, `EXPIRED` |
  | `REQUIRES_CUSTOMER_RENEWAL_IN_GRACE` | `ISSUED`, `REQUIRES_CUSTOMER_RENEWAL`, `REQUIRES_POLICY_REISSUANCE`, `EXPIRED` |
  | `REQUIRES_CUSTOMER_RENEWAL` | `ISSUED`, `REQUIRES_POLICY_REISSUANCE`, `EXPIRED` |
  | `REQUIRES_POLICY_REISSUANCE`, `EXPIRED`, `REJECTED` | terminal |

  Only `APPROVED → ISSUED` carries a body payload. Every other transition is a payload-free status flip that preserves the stored `external_policy_id`/dates, so a renewal-state → `ISSUED` reactivation keeps the original carrier reference.
...
stateDiagram-v2
    ...
    APPROVED --> ISSUED: issue (payload)
    ISSUED --> REQUIRES_CUSTOMER_RENEWAL_IN_GRACE
    ISSUED --> REQUIRES_CUSTOMER_RENEWAL
    ISSUED --> REQUIRES_POLICY_REISSUANCE
    ISSUED --> EXPIRED
    REQUIRES_CUSTOMER_RENEWAL_IN_GRACE --> ISSUED
    REQUIRES_CUSTOMER_RENEWAL_IN_GRACE --> REQUIRES_CUSTOMER_RENEWAL
    REQUIRES_CUSTOMER_RENEWAL_IN_GRACE --> REQUIRES_POLICY_REISSUANCE
    REQUIRES_CUSTOMER_RENEWAL_IN_GRACE --> EXPIRED
    REQUIRES_CUSTOMER_RENEWAL --> ISSUED
    REQUIRES_CUSTOMER_RENEWAL --> REQUIRES_POLICY_REISSUANCE
    REQUIRES_CUSTOMER_RENEWAL --> EXPIRED
...
```json Create Response 201
{
  ...
  "master_policy_number": "NY00001-100001-001",
  "dependant_ids": [
...
| `IP_1021` | 400 | Invalid status transition on `PATCH .../status` — the `from`/`to` pair is not allowed |
