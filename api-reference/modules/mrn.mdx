...
<Note>
  MRN records are also **auto-created** during insurance policy creation — one per covered dependant at the benefit's provider. See the [Insurance Policy module](/modules/insurance_policy) for details.
</Note>
...
| Endpoint | App (JWT) user | Benefit provider | Admin / platform | Notes |
|----------|----------------|------------------|------------------|-------|
| `POST /mrns` | ✓ | ✓ | ✓ | App user: dependant must belong to the token user. No route-level gate — scoping is enforced in core |
| `GET /mrns` | ✓ own only | ✓ own provider | ✓ | App user is force-scoped to their own `primary_user_id`; a benefit provider is force-scoped to its own `benefit_provider_id` |
| `GET /mrns/{id}` | ✓ own only | ✓ own provider | ✓ | Ownership / provider-scope mismatch returns 404 (existence not leaked); also 404 if inactive |
| `PATCH /mrns/{id}` | — | ✓ own provider | ✓ | Coarse gate (`require_benefit_provider`) then core strict-checks the MRN's `benefit_provider_id`. Replaces `external_mrn` only |
| `DELETE /mrns/{id}` | — | ✓ | ✓ | Trusted-backend only (admin / platform / benefit provider). Sets `status → INACTIVE` |
...
<CardGroup cols={2}>
  ...
  <Card title="GET /mrns" icon="list" color="#3b82f6" href="/api/endpoints/mrns/list">
    List MRNs. Filter by `dependant_id`, `benefit_provider_id`, `primary_user_id`, `statuses`, or `mrn`.
  </Card>
  ...
  <Card title="PATCH /mrns/{id}" icon="pen-to-square" color="#0ea5e9" href="/api/endpoints/mrns/update">
    Replace the `external_mrn` JSONB. Admin or the owning benefit provider. Used to backfill `mrn` once the provider issues it.
  </Card>
  <Card title="DELETE /mrns/{id}" icon="trash" color="#dc2626" href="/api/endpoints/mrns/delete">
    Soft-delete (`status → INACTIVE`). Trusted-backend caller (admin / platform / benefit provider).
  </Card>
</CardGroup>
...
|------|------|-------------|
| `MR_1100` | 500 | Internal server error |
| `MR_1101` | 404 | MRN not found or inactive |
| `MR_1102` | 403 | JWT user accessing an MRN belonging to another user's dependant |
| `MR_1103` | 409 | MRN already exists for this dependant + provider combination |
| `MR_1104` | 404 | Dependant not found or inactive |
| `MR_1105` | 404 | Benefit provider not found or inactive |
| `MR_1106` | 400 | Invalid UUID in filter or request body |
