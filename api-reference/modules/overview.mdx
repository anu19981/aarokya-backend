...
description: "Aarokya is composed of nine domains: Auth, Platform, User, Benefit Provider, Benefit, Dependant, Account Provider, Account, and MRN. Each owns its own tables, business logic, and error types."
---
...
<CardGroup cols={3}>
  ...
  <Card title="Account Provider" icon="building-columns" color="#f59e0b" href="/modules/account_provider">
    **Admin-only**

    Financial institutions (banks, HSA custodians) that back user accounts. Delete guarded by child accounts.

    **Tables:** `account_providers`
  </Card>
  <Card title="Account" icon="credit-card" color="#16a34a" href="/modules/account">
    **JWT self + Partner key**

    Links a user to an account provider with an external account ID. Responses include denormalized provider fields via JOIN.
