...
|--------|--------|---------|
| `Authorization: Bearer <admin-jwt>` | JWT (admin scope, via OIDC) | Internal ops — CRUD for platforms, providers, benefits |
| `api-key` | Static secret (per partner) | External partners — token issuance, account/MRN ops |
| `Authorization: Bearer <user-jwt>` | JWT (HS256) | Logged-in users — profile, dependants, accounts, MRNs |
