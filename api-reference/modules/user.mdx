...
<CardGroup cols={2}>
  ...
  <Card title="DELETE /users/{user_id}" icon="trash" color="#dc2626" href="/api/endpoints/users/delete">
    **Admin only.** Soft-delete a user (sets `status → deactivated`). Deactivated users cannot receive new tokens.
  </Card>
</CardGroup>
...
`DELETE /users/{user_id}` sets `status → deactivated` on the user row — the record is never physically removed. A deactivated user's phone number can be re-registered (a new user row is created on the next `POST /auth/token`).
